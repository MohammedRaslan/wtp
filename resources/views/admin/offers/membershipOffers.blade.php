<form method="post" class="membershipOfferForm omForm" enctype="multipart/form-data" style="width: 56%;" >
{{ csrf_field() }}
<input type="hidden" name="id">
  ...
  <h4>New Offer? </h4>
    ...
    <div class="col-auto">
      <input type="number" class="form-control mb-2" name="duration" placeholder="Extra dutation">
    </div>
    <div class="col-auto">
      <input type="number" class="form-control mb-2" name="storage" placeholder="Extra storage MB">
    </div>
    <div class="col-auto">
...
<table class="table table-striped " style="width: 84%;">
  ...
  <tbody class="membershipOffersTable">
  @foreach($offers as $offer)
  <tr id="{{$offer->id}}">
      <td><p>{{$offer->title}}</p></td>
      <td><p>{{$offer->discount}}</p></td>
      <td>
        <input type='button' class='btn btn-primary membershipOfferInfo' id="{{$offer->id}}" value='Info'/>
        <input type='button' class='btn btn-danger deleteMembershipOffer' id="{{$offer->id}}"  value='Delete'/>
      </td>
  </tr>
  @endforeach
